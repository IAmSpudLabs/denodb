...
Deno.test("SQLite: Response model, Query Null Fields", async () => {
  ...
  const selectNullFieldResponseChain = await Article.where("title", "hola")
    .whereNull("content").all();
  ...
  const selectNotNullFieldResponseChain = await Article.where("title", "hola")
    .whereNotNull("content").all();
